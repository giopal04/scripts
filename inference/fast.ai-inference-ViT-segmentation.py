...
def parse_args():
    ...
    parser.add_argument('--encoder',		type=str, default='vit_giant_patch14_reg4_dinov2',	help='ViT encoder to use')
    parser.add_argument('--decoder',		type=str, default='vit_small_patch14_reg4_dinov2',	help='ViT decoder to use (only if segmenter version==2)')
    parser.add_argument('--segmenter_version',	type=str, default='2',					help='Segmenter version: 1: symmetric encoder/decoder, 2: asymmetric')
...
def main():
	...
	# Load model
	learn = load_model(input_dir, args.model_path, device, encoder=args.encoder, img_size=args.img_size, num_classes=args.num_classes, args=args, task=args.task, decoder=args.decoder)
	learn.model.eval()
	...
	for i, batch in enumerate(dl):
		print(f"{Text(batch[0], 'batch[0]'):inspect}")
		start = i * args.batch_size
		end = start + len(batch[0])
		batch_files = files[start:end]
		process_batch(batch[0], learn.model, batch_files, output_dir, device, args, debug=args.debug)
